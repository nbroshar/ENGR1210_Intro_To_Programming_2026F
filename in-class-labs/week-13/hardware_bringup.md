...
- `blink_test.py`      — run this FIRST: proves the board/emulator works
...
**Step 0 — blink test (always do this first).** Before the translator, prove
the board itself works. In Wokwi, put the contents of `blink_test.py` into
`main.py` (Wokwi auto-runs `main.py`) and press Play. You should see the
onboard LED blink once a second and `blink 1`, `blink 2`, ... in the console.
That confirms the board, firmware, and your files all work. Now replace
`main.py` with the real entry point and continue:
...
- **Blink test first:** copy `blink_test.py` to the board (in Thonny, open it
  and press Run; or `mpremote connect auto run blink_test.py`). Confirm the
  onboard LED blinks before you copy anything else. If it doesn't, the problem
  is the board/firmware/cable -- solve that before touching the translator.
